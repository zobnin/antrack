...
public class Keys {
    ...
    public static void saveKeys() {
        if (privateKey == null || publicKey == null) {
            if (!new File(Init.MAIN_DIR + C.PUBLIC_KEY_FILE).exists() ||
                    !new File(Init.APP_DIR + C.PRIVATE_KEY_FILE).exists()) {
                try {
    ...
    public static PrivateKey getPrivateKey() {
        if (privateKey == null) {
            try {
                String stringKey = Files.readTextFile(Init.APP_DIR + C.PRIVATE_KEY_FILE);
